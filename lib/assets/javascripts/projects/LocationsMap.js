  ...
  var LocationsMap = AddonWidget.extend({
    ...
    templates: {
      'option': '<option value="<%= values %><% if (values != text) { %>(<%= text %>)<% } %>" selected><%= text %></option>'
    },
    ...
    initialize: function() {
      ...
      _.bindAll(this, '_initMap', '_add', '_remove', '_onMapGeocoder', '_onMapClick', '_addNewPoint', '_addMarker', '_onClickAdd');
    ...
    _add: function(m) {
      ...
      // Editon in the widget, let's advice form view
      this.trigger('edited');
    },
